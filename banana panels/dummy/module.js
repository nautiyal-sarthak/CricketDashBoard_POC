...
define([
  ...
  function(angular, app, _, $, kbn, d3, d3tip) {
    ...
    module.controller('dummy', function($scope, querySrv, dashboard, filterSrv) {
      $scope.panelMeta = {
        modals: [{
          description: "Inspect",
          icon: "icon-info-sign",
      ...
      $scope.get_data = function() {
        ...
        var wt_json = '&wt=json';
        var rows_limit = '&rows=10'; // for terms, we do not need the actual response doc, so set rows=0
        ...
        results.then(function(results) {
          ...
          _.each(results.response.docs, function(v) {
			  var slice = {
                  letter: v[$scope.panel.field][0],
                  frequency: v[$scope.panel.fl][0]
				  };
    ...
    module.directive('dummyChart', function() {
      return {
        restrict: 'A',
            ...
            var height = parseInt(scope.row.height);
             ...
             var margin = {top: 40, right: 20, bottom: 60, left: 40};
              ...
              var tip = d3tip()
                  ...
                  .offset([-10, 0])
                  .html(function(d) {
                      return "<strong>Runs:</strong> <span style='color:red'>" + d.frequency + "</span>";
                  });
